...
class Block_encoder_with_skip(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr, img_size):
        ...
        self.layernorm = nn.LayerNorm([in_channels, img_size, img_size])
        self.layer1 = nn.Sequential(
            nn.LazyConv2d(in_channels, kernel_size=3, stride=1, padding="same"),
            # nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding="same"),
            nn.LeakyReLU(1e-2)
        )
        self.layer2 = nn.Sequential(
...
class FCT(nn.Module):
    def __init__(self, image_size=512, in_channels=3, num_classes=2):
        ...
        self.block_1 = Block_encoder_without_skip(in_channels, filters[0], att_heads[0], dpr[0], self.img_size)
        ...
        self.ds7 = DS_out(filters[6], num_classes, self.img_size // 8)
        self.ds8 = DS_out(filters[7], num_classes, self.img_size // 4)
        self.ds9 = DS_out(filters[8], num_classes, self.img_size // 2)
...
def init_weights(m):
    ...
    """
    if isinstance(m, nn.Conv2d) and not isinstance(m, nn.LazyConv2d):

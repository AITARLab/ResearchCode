...
def save_txt(content: list, save_path: str):#将列表中的内容写入到指定路径的文本文件中
    """
...
def split_data(data_root: str, Plist:list, Nlist:list, test_percent: float, fold: int = 0, seed=41):
#划分训练集和测试集，保存相应的数据文件
    min_number = min(len(Plist), len(Nlist))
    test_number = int(test_percent * min_number)
    # print(f"test_number:{test_number}")
    random.seed(seed)
    test_P = random.sample(Plist, test_number)
    test_N = random.sample(Nlist, int(test_number/3))
    print(f"test Positive patient number:{len(test_P)},test Negative patient number:{len(test_N)}")
    test = test_N + test_P
    ...
    print(f"train_p img_count:{train_p_count}, train_n img_count:{train_n_count}")
    print(f"test_p img_count:{test_p_count},test_n img_count:{test_n_count}")
...
def split_data_rupture(data_root: str, Plist:list, test_percent: float, seed: int =41, fold: int = 0):
    number = len(Plist)
    ...
    test_log = "log/rupture_inter_set.txt"
    save_txt(test_data, test_log)
    ...
    for train_index, val_index in kf.split(train_data):
        train_fold, val_fold = [train_data[i] for i in train_index], [train_data[i] for i in val_index]
        train_log = f"log/rupture {fold} fold train.txt"
        val_log = f"log/rupture {fold} fold val.txt"
        fold += 1
        save_txt(train_fold, train_log)
        save_txt(val_fold, val_log)
...
def seg_loader(img_path, mask_path, mode=None):#加载图像和mask，进行预处理并返回
    """
    ...
    if mode == "train":
        spleen_mask_root = img_path.replace("images", "spleen_mask").split(".")[0]
        types = ['JPG', 'jpg', 'PNG', 'png']
        for type in types:
                spleen_mask_path = spleen_mask_root + "." + type
                if os.path.exists(spleen_mask_path) == True:
                    break
        spleen_mask = np.array(Image.open(spleen_mask_path))
        mask_array = spleen_mask[:, :, np.newaxis]
...
class img_dataset(td.Dataset):#数据集类，加载图像数据集，返回图像和mask

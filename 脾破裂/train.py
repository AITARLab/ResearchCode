...
import torch.optim
from Model import fcn_resnet, Unet, Network_Res2Net_GRA_NCD
...
def seg_train(model_name, fold, device, task, model_type=None, continue_train=False):
    ...
    if task == "spleen_seg":
        ...
        start = "log/rupture "
        inter_log = "log/rupture_inter_set.txt"
        mode = None if task == "one_step_seg" else "train"
    ...
    for i in range(fold):
        ...
        train_str = start + "{} fold train.txt".format(i)
        val_str = start + "{} fold val.txt".format(i)
        ...
        train_data = img_dataset(data_image=train_img, data_mask=train_mask, mode=mode)
        val_data = img_dataset(data_image=val_img, data_mask=val_mask, mode=mode)
